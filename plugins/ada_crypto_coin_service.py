...
def derive_child_keys(parent_node, path, private):
    ...
    BIP32Ed25519_class = BIP32Ed25519()
    for i in path.split('/'):
        if i.endswith("'"):
            i = int(i[:-1]) + 2**31
        else:
...
class CardanoCoinService(CoinService):
    ...
    def __generate_address_from_seed(self, mnemonic):
        ...
        masterkey = generateMasterKey_Icarus(mnemonic=mnemonic, passphrase=passphrase.encode(),
        ...
        account_node = derive_child_keys(masterkey, "1852'/1815'/0'", True)
        ...
        spend_node = derive_child_keys(account_node, "0/0", False)
        (AP, cP) = spend_node
        # print("Spending Key")
        # print("AP:", AP.hex())
        # print("cP:", cP.hex())
        ...
        stake_node = derive_child_keys(account_node, "2/0", False)
        (AP, cP) = stake_node
        # print("Staking Key")
        # print("AP:", AP.hex())
        # print("cP:", cP.hex())
        ...
        out_data = convertbits(data, 8, 5)

        encoded_address = bech32_encode("addr", out_data)
